...
class LispTest(unittest.TestCase):
    ...
    def _compare_outputs(self, x, y, msg):
        self.assertEqual(x['ok'], y['ok'], msg=msg)
        if x['ok']:
            if isinstance(x['output'], (int, float)):
                self.assertAlmostEqual(x['output'], y['output'], msg=msg)
            else:
                self.assertEqual(x['output'], y['output'], msg=msg)
        else:
    ...
    @staticmethod
    def _test_num_msg(n):
        msg = "\nfor test_inputs/"+str(n)+".jsn"
        try:
            with open('carlae_code/%s.crl' % n) as f:
                code = f.read()
            msg += " and carlae_code/"+str(n)+".crl"
        except Exception as e:
            with open('test_inputs/%s.json' % n) as f:
