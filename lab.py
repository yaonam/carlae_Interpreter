...
def mapp(tree):
    ...
    func, args = tree
    if not callable(func) or (type(args)!=Pair and args!=None):
        raise EvaluationError
    result = listt(None) # Start with an empty list
    if args!=None:
        for each in args:
...
if __name__ == '__main__':
  ...
  [
    ...
    envir = env()
    for name in sys.argv[1:]: # Evaluate the given files
